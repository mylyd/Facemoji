...
object StickerPackLoader {
    ...
    fun fetchStickerPacks(context: Context): ArrayList<StickerPack> {
        val cursor = context.contentResolver.query(
            StickerContentProvider.AUTHORITY_URI, null, null, null, null)
            ?: throw IllegalStateException("could not fetch from content provider, " + BuildConfig.CONTENT_PROVIDER_AUTHORITY) //create stickerpacks contentProvider
        val identifierSet = HashSet<String?>()
        val stickerPackList =
            fetchFromContentProvider(cursor) //从ContentProvider读取一个个表情包的属性字段，再通过构造函数创建StickerPack(不含Stickers)
        for (stickerPack in stickerPackList) {
            ...
            identifierSet.add(stickerPack.identifier) //确保表情包是唯一的
        }
    ...
    fun getStickersForPack(context: Context, stickerPack: StickerPack): List<Sticker> {
        val stickers =
        ...
        for (sticker in stickers) {
            ...
            try {
                ...
                throw IllegalStateException("Asset file doesn't exist. pack:${stickerPack.name}, sticker: ${stickerPack.name}", e)
            }
        }
        return stickers
    }

    //从ContentProvider读取一个个表情包的属性字段，再通过构造函数创建StickerPack(不含Stickers)
    private fun fetchFromContentProvider(cursor: Cursor): ArrayList<StickerPack> {
        ...
        do {
            ...
            val stickerPack = StickerPack(identifier, name, publisher, trayImage, publisherEmail,
    ...
    private fun fetchFromContentProviderForStickers(identifier: String?, contentResolver: ContentResolver): List<Sticker> {
        ...
        val cursor = contentResolver.query(uri, projection, null, null, null) //create sticker contentProvider
        ...
        if (cursor != null && cursor.count > 0) {
            ...
            do {
                ...
                if (!TextUtils.isEmpty(emojisConcatenated)) {
                    emojis = listOf(*emojisConcatenated.split(",".toRegex()).toTypedArray()) }
                stickers.add(Sticker(name, emojis as List<String>?))
            } while (cursor.moveToNext())
        }
        cursor?.close()
        return stickers
        ...
        contentResolver.openInputStream(getStickerAssetUri(identifier, name))
            .use { inputStream ->
                ByteArrayOutputStream().use { buffer ->
                    ...
                    var read: Int
                    val data: ByteArray = ByteArray(16384)
    ...
    @Throws(IOException::class)
    fun fetchFileByte(fileName: String): ByteArray? {
        ...
        return try {
            ...
            Log.e("StickerPackLoader", "file  not found:$fileName")
            e.printStackTrace()
            null
        }
    }
